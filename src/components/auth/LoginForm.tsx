...
export function LoginForm() {
  ...
  const [oauthReady, setOauthReady] = useState({ google: true, facebook: true });
  ...
  useEffect(() => {
    api.auth.oauthProviders().then((res) => {
      if (res.success && res.data) setOauthReady(res.data);
    });
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.needsVerification) {
      setVerifyMode(true);
      setError(result.error || "Email chưa xác nhận");
      const resent = await resendVerification(email.trim());
      if (resent.demoCode) setDemoCode(resent.demoCode);
      return;
    }
      ...
      <form onSubmit={handleVerify} className="space-y-4">
        <div className="rounded-xl bg-white p-6 shadow-card border border-gray-100 space-y-4">
          <p className="text-sm text-gray-600">
            Tài khoản <strong>{email}</strong> chưa xác nhận email. Nhập mã 6 số
            đã gửi tới hộp thư.
          </p>
          {demoCode && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
              Mã demo: <strong className="tracking-widest">{demoCode}</strong>
            </div>
          )}
          {error && (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <a
          ...
          className={cn(
            "flex items-center justify-center gap-2 rounded-lg border border-gray-200 bg-white py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors",
            !oauthReady.google && "opacity-70"
          )}
        >
        ...
        <a
          ...
          className={cn(
            "flex items-center justify-center gap-2 rounded-lg border border-gray-200 bg-white py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors",
            !oauthReady.facebook && "opacity-70"
          )}
        >
      ...
      {(!oauthReady.google || !oauthReady.facebook) && (
        <p className="text-center text-xs text-gray-400">
          Nút Google/Facebook cần thêm Client ID trên Vercel (.env) mới đăng nhập được.
        </p>
      )}
